...
//   },[messages])
  ...
  return (
    <>
      <div className="ChatBox-container" style={{border:'1px solid purple',display:'flex',flexDirection:'column',padding:'1rem',borderRadius:'10px'}}>
        {chat ? (
          <>
            <div className="chatHeader">
              <div
                className="follower"
                 style={{display:'flex',justifyContent:'flex-start',gap:'0.5rem',marginBottom:'0.5rem'}}
              >
            ...
            style={{ alignSelf: "baseline",maxHeight:'350px',minHeight:'350px',overflow:'scroll',display:'flex',flexDirection:'column-reverse'}}>
              {messages.map((message) => (
                <div style={{minWidth:'450px'}}>
                  {message.senderId === currentUserId?
                    (<div
                    className="message own" style={{display:'flex',justifyContent:'flex-end',flexDirection:'column',margin:'0.2rem'
                    ,backgroundColor:'#e1bee7'
                    ,borderRadius:'20px',padding:'0.5rem'
                }}
                  >
                    <span style={{alignSelf:'end',fontSize:'14px',fontWeight:'500'}}>{message.text}<br /></span>
                    <span style={{alignSelf:'end',fontSize:'10px'}}>
                      {timeAgo.format(new Date(message.createdAt))}
                    </span>
                  </div>):
                    (<div style={{display:'flex',justifyContent:'flex-start',flexDirection:'column',margin:'0.2rem'
                    ,backgroundColor:'#c8e6c9',borderRadius:'20px',padding:'0.5rem'
                }}
                    className="message"
                  >
                    <span style={{alignSelf:'start',fontSize:'14px',fontWeight:'450'}}>{message.text}<br /></span>
          ...
          <Typography className="chatbox-empty-message" sx={{marginTop:'70px'}}>Tap on a Chat to start conversation</Typography>
        )}
      </div>
    </>
  );
